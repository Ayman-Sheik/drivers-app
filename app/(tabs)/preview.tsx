...
import { Picker } from '@react-native-picker/picker'; // make sure this is imported at top
...
export default function PreviewScreen() {
  ...
  const generatePDF = async () => {
    if (!uri || !filename.trim()) {
      Alert.alert('Missing Info', 'Please provide a filename and scan an image.');
      return;
    }
    ...
    try {
      const html = `
        <html>
          <body style="font-family: Arial; padding: 24px;">
            ...
            <img src="${uri}" alt="Scanned Document" style="width: 100%; margin-top: 16px;" />
          </body>
        </html>
      `;
      ...
      const newPdfPath = `${FileSystem.documentDirectory}${filename.trim()}.pdf`;
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#121212] px-4">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        <Text className="text-white text-2xl font-bold my-4 text-center">Preview & Save</Text>
...
>
  ...
  />
        ...
        >
          <Text className="text-black text-center font-bold text-lg">Generate PDF</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
